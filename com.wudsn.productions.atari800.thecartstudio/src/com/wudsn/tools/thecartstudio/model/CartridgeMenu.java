...
public final class CartridgeMenu {
	...
	public static final int MENU_ENTRY_LENGTH = 64; // Must be a divisor of
	// $2000
	...
	/**
	 * Collector callback to get the entries from an importable cartridge menu into
	 * the workbook menu.
	 * 
	 */
	private final class InnerMenuCollector implements Collector {
		...
		public void collectMenu(Object owner, String info) {
			...
			if (logMenus) {
				Log.logInfo("Found importable menu: {0} - '{1}' - '{2}'",
						new Object[] { workbookEntry.getContentType().toString(), info, workbookEntry.getTitle() });
			}
		...
		public final void collectMenuEntry(Object owner, int itemMenuVersion, int itemNumber, String title) {
			...
			if (StringUtility.isSpecified(title)) {
				...
				workbookMenuEntry.setSourceType(WorkbookMenuEntry.SOURCE_TYPE_MENU_ENTRY_ITEM);
	...
	private CartridgeMenu(CartridgeMenuType cartridgeMenuType) {
		...
		if (jarFolder != null) {
			try {
				...
				externalSimpleMenuContent = FileUtility.readBytes(externalSimpleMenuFile, FileUtility.MAX_SIZE_1MB,
						true);
		...
		simpleMenuContent = ResourceUtility.loadResourceAsByteArray(SIMPLE_MENU);

		// Override with external file?
		if (externalSimpleMenuContent != null && externalSimpleMenuContent.length == simpleMenuContent.length) {
			simpleMenuContent = externalSimpleMenuContent;
			external = true;
		}

		if (simpleMenuContent != null && simpleMenuContent.length > 0x2000) {
			String signature = new String(simpleMenuContent, Offsets.CARTMENU_SIGNATURE, 16);
			if (signature.startsWith("The!Cart")) {
				version = signature.substring(8, 12) + "-" + signature.substring(12, 14) + "-"
		...
		if (jarFolder != null) {
			try {
				...
				externalExtendedMenuContent = FileUtility.readBytes(externalExtendedMenuFile, FileUtility.MAX_SIZE_1MB,
						true);
			} catch (CoreException ignore) {
			}
		}

		extendedMenuContent = ResourceUtility.loadResourceAsByteArray(EXTENDED_MENU);

		if (externalExtendedMenuContent != null && externalExtendedMenuContent.length > 0x2000) {
		...
		if (valid) {
			...
			if (root.getCartridgeMenuType() == CartridgeMenuType.SIMPLE) {
				...
				if (content != null) {
					...
					System.arraycopy(simpleMenuContent, 0, extendedMenuContent, 0, Offsets.MENU_STUDIO_ROM);
					System.arraycopy(simpleMenuContent, Offsets.CARTMENU_SIGNATURE, extendedMenuContent,
							Offsets.CARTMENU_SIGNATURE, Offsets.CARTMENU_SIGNATURE_LENGTH);
					...
					System.arraycopy(simpleMenuContent, Offsets.CARTMENU_FIRST_BANK_END, extendedMenuContent,
							Offsets.CARTMENU_FIRST_BANK_END,
							simpleMenuContent.length - Offsets.CARTMENU_FIRST_BANK_END);

					// Set MCB content.
					int startBankNumber = root.getMenuEntriesContentProvider().getStartBankNumber();
					setByte(content, Offsets.CARTRIDGE_TYPE, root.getCartridgeType().getNumericId());
					setWord(content, Offsets.BANK_COUNT, root.getBankCount());
					// +1 for "All"
					setWord(content, Offsets.MENU_GENRES_COUNT, root.getUnmodifiableGenresList().size() + 1);
					setWord(content, Offsets.MENU_GENRES_START_BANK_NUMBER, startBankNumber + 0);
					setWord(content, Offsets.MENU_ENTRIES_COUNT, root.getMenuEntriesList().size());
					setWord(content, Offsets.MENU_ENTRIES_START_BANK_NUMBER, startBankNumber + 1);
					...
					setString(content, Offsets.MENU_TITLE, title.toString(), WorkbookEntry.TITLE_LENGTH);
				}
			}
		}
	...
	private void createMenuEntries(Workbook workbook, WorkbookExport workbookExport, MessageQueue messageQueue) {
		...
		for (int i = 0; i < workbookEntries.size(); i++) {
			...
			if (workbookEntry.getDisplayMode().equals(DisplayMode.MULTIPLE_ENTRIES)) {
				importableMenu = workbookEntry.getContentType().createImportableMenu(new byte[0]);
				if (importableMenu == null) {
					// ERROR: Content type '{0}' does not support display mode
					// '{1}'.
					messageQueue.sendMessage(workbookEntry, WorkbookEntry.Attributes.DISPLAY_MODE, Messages.E422,
							workbookEntry.getContentType().toString(), DisplayMode.MULTIPLE_ENTRIES.toString(),
							ContentType.getImportableMenuList());
					continue;
				}
			}
			...
			if (importableMenu != null) {
				...
				if (content != null) {
					...
					case Result.NO_MENU_ENTRIES_FOUND:
						...
						// can add
						// it.
						messageQueue.sendMessage(workbookEntry, WorkbookEntry.Attributes.DISPLAY_MODE, Messages.E423);
						break;
					case Result.MENU_ENTRIES_FOUND_BUT_NOT_STARTABLE:
						...
						messageQueue.sendMessage(workbookEntry, WorkbookEntry.Attributes.DISPLAY_MODE, Messages.I425,
	...
	public static void setMenuEntriesContent(Workbook workbook, byte[] result, MessageQueue messageQueue) {
		...
		for (int i = 0; i < workbookMenuEntries.size(); i++) {
			...
			try {
				...
				setByte(result, offset, workbookEntry.getContentType().getTheCartMode());
				...
				setByte(result, offset, workbookEntry.getFavoriteIndicator() ? 0x80 : 0x00);
				...
				throw new RuntimeException("Exception udring processing of entry " + workbookEntry.getTitle() + " for "
						+ workbookEntry.getFileName(), ex);
			}
